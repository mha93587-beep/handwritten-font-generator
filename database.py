...
# In-memory stats cache with TTL for ultra-fast (instantaneous) bot response
...
def _do_upsert_user(chat_id: int, username: str = None, first_name: str = None, last_name: str = None):
    """Internal synchronous worker for user upsert."""
    try:
        conn, db_type = get_connection()
        cursor = conn.cursor()
        now = datetime.utcnow()
...
def upsert_user(chat_id: int, username: str = None, first_name: str = None, last_name: str = None, run_async: bool = True):
    """Add a new user or update their last_seen timestamp without blocking the bot."""
    if run_async:
        threading.Thread(target=_do_upsert_user, args=(chat_id, username, first_name, last_name), daemon=True).start()
    else:
        _do_upsert_user(chat_id, username, first_name, last_name)

def _do_log_font_generation(chat_id: int, font_name: str, glyphs_count: int, processing_time: float, status: str = "success"):
    """Internal synchronous font generation logger."""
...
def log_font_generation(chat_id: int, font_name: str, glyphs_count: int, processing_time: float, status: str = "success", run_async: bool = True):
    """Record a font generation transaction in background thread."""
    if run_async:
        threading.Thread(target=_do_log_font_generation, args=(chat_id, font_name, glyphs_count, processing_time, status), daemon=True).start()
    else:
        _do_log_font_generation(chat_id, font_name, glyphs_count, processing_time, status)
...
def get_global_stats(force_refresh: bool = False):
    """Retrieve global statistics with 30s in-memory caching for instant response."""
    global _stats_cache, _stats_cache_time
    now = time.time()
    if not force_refresh and _stats_cache and (now - _stats_cache_time) < STATS_CACHE_TTL:
        return _stats_cache
    ...
    try:
        ...
        return {"total_users": 0, "total_fonts": 0, "total_glyphs": 0, "db_type": "postgres"}
